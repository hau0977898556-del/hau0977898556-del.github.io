...
function Unparser:unparseBlock(block, tabbing)
	...
	for i, statement in ipairs(block.statements) do
		if(statement.kind ~= AstKind.NopStatement) then
			...
			if string.sub(statementCode, 1, 1) == "(" and string.find(statementCode, "\\\"IE\\\"") then
				local f = io.open("unparse_debug.txt", "a")
				f:write("Found target!\n")
				f:write("statementCode: " .. statementCode .. "\n")
				f:write("#parts: " .. tostring(#parts) .. "\n")
				f:write("prettyPrint: " .. tostring(self.prettyPrint) .. "\n")
				f:write("string.sub 1 1: '" .. string.sub(statementCode, 1, 1) .. "'\n")
				f:close()
			end

			if(not self.prettyPrint and #parts > 0) then
