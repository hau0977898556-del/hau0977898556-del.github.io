...
return function(self, expression, funcDepth, numReturns)
    ...
    self:addStatement(self:setRegister(scope, self.POS_REGISTER, Ast.OrExpression(Ast.AndExpression(self:register(scope, lhsReg), self:crazyExpr(scope, Ast.NumberExpression(block2.id))), self:crazyExpr(scope, Ast.NumberExpression(block1.id)))), {self.POS_REGISTER}, {lhsReg}, false);
    self:freeRegister(lhsReg, false);
